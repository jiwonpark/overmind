...
async def update_charts(type, action = None, service = None, loop = None):
    ...
    for symbol in candidates:
        ...
        if action is not None:
            await action(service, symbol, last_candle, loop)
        # print(symbol)
...
async def test(service, symbol, *_):
    chart = charts[symbol]
...
async def engage(service, symbol, last_candle, loop):
    type = '1m'
    chart = charts[symbol]
    now = time.time()
    ...
    #     # await asyncio.sleep(0)
    if not service.history_condition(chart):
        return
    ...
    this_candle_match = service.this_candle_condition(last_candle, chart)
